...
ROOT_USER_PASSWORD=linux

DEFAULT_LINUX_USER=tux
DEFAULT_LINUX_USER_PASSWORD=linux

ENABLED_SERVICES_LIST="xrdp.service sshd.service"
...
LOGFILE=/var/log/install_gui_vm_with_guacamole.log
...
install_base_packages() {
  ...
  case ${ID} in
    opensuse-leap)
      zypper ref
      zypper --non-interactive in -t pattern enhanced_base gnome kvm_server kvm_tools yast2_basis yast2_desktop yast2_server 
      zypper --non-interactive in yast2-rdp xrdp nmap podman git-core
    ;;
    sles)
      zypper ref
      zypper --non-interactive in -t pattern enhanced_base gnome_basic kvm_server kvm_tools yast2_basis yast2_desktop yast2_server 
      zypper --non-interactive in yast2-rdp xrdp nmap podman git-core
    ;;
  esac
}
...
enable_required_services() {
  for ENABLED_SERVICE in ${ENABLED_SERVICES_LIST}
  do
    systemctl enable --now ${ENABLED_SERVICE}
  done
...
# Create Default Linux User
create_default_linux_user() {
  if ! [ -z ${DEFAULT_LINUX_USER} ]
  then
    useradd -m ${DEFAULT_LINUX_USER}

    if ! [ -z ${DEFAULT_LINUX_USER_PASSWORD} ]
    then
      echo "${DEFAULT_LINUX_USER}:${DEFAULT_LINUX_USER_PASSWORD}" | chpasswd
    fi
  fi
}
